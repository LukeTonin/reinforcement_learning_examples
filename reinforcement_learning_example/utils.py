...
def run_agent_in_env(
    env_factory: Callable[[None], gym.Env], agent: Agent = None, num_episodes_to_show: int = 3
) -> None:
    """"""
    env = env_factory()

    try:
        for _ in range(num_episodes_to_show):
...
def plot_learning_curve(
    x: list, scores: List[float], epsilons: List[float], filename: str = None, lines: List[float] = None
):
    fig = plt.figure()
    ...
    for t in range(N):
        running_average[t] = np.mean(scores[max(0, t - 20) : (t + 1)])
